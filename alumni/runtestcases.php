...
$query = "SELECT * FROM alumni where Geek_Email='".$_SESSION['progeeks_uid']."' ";
...
if($error==''){
	...
	$in1="";
	$in2="";
	echo(shell_exec("timeout --signal=SIGKILL 1 ./a.out < ".$infile." >".$outfile." 2>&1"));
	...
	$sp = "alumni/".$_SESSION['progeeks_uid'];
...
if(isset($_POST['txt1'])){	
	...
	if($data=='')
		...
		while( $num <=10 &&$row[$input] != NULL && $row[$output] != NULL){
			...
			{
			$query="update alumni set Geek_Testcase=".$count.",Geek_Time='".date('Y-m-d H:i:s',time())."' where Geek_Email='".$_SESSION['progeeks_uid']."'";
			$res1 = $con->query($query);
			}
			unlink($outfile);
 		...
 		echo "</pre>";
		if($count == 10){
			$query = "SELECT Geek_Level FROM alumni where Geek_Email='".$_SESSION['progeeks_uid']."' and Geek_Level='".$row['Question_Id']."'";
			$res1 = $con->query($query);
			if($row1 = $res1->fetch_assoc())
			{
				
				$query="update alumni set Geek_Level=Geek_Level+1,Geek_Testcase=0,Geek_Time='".date('Y-m-d H:i:s',time())."' where Geek_Email='".$_SESSION['progeeks_uid']."'";
					$res1 = $con->query($query);
